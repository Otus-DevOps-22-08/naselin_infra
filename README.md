...
1. Установлены VirtualBox и Vagrant, описана локальная инфраструктура.
2. В Vagrantfile добавлены provision'еры (заработало без `base.yml`, но, пришлось его добавить для прохождения автотестов).
3. Доработаны роли `db` и `app` (таски разнесены по разным файлам, добавлены теги).
4. Добавлено проксирование через `ansible.extra_vars` (`nginx_sites`).
5. Установлена `Molecule`, с помощью модулей `Testinfra` протестирована роль `db`.
